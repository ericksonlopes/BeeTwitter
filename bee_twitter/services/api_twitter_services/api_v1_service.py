...
class APITwitterV1Service:
    ...
    def get_user_id(self, screen_name: str) -> int:
        try:
            with ConnectorSnowflake() as session:
                ...
                self.get_user_by_screen_name(screen_name=screen_name)
    ...
    def get_user_by_screen_name(self, screen_name: str) -> UserModel:
        try:
            with ConnectorSnowflake() as session:
                user = session.query(UserModel).filter(UserModel.screen_name == screen_name).first()
    ...
    def get_user_by_id(self, id_author) -> UserModel:
        try:
            with ConnectorSnowflake() as session:
                user = session.query(UserModel).filter(UserModel.id == id_author).first()

                if user:
                    logger.info(f'User {id_author} already exists')
                    return user
        except Exception as e:
            logger.error(f'Error on get user: {e}')

        try:
            data = self.__api().get_user(id=id_author).__dict__["_json"]

            logger.info(f'User {id_author} found with success')
            ...
            logger.info(f'User {id_author} saved with success')
